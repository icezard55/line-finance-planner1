...
async function seedDefaults(userId) {
  ...
  await pool.query(
    `INSERT INTO finance.categories (user_id, name, type) VALUES
       ($1, 'เงินเดือน/ค่าคอม', 'income'),
       ($1, 'รายได้ออนไลน์/ลงทุน', 'income'),
       ($1, 'รับชำระลูกหนี้', 'income'),
       ($1, 'อาหาร', 'expense'),
       ($1, 'ค่าเดินทาง', 'expense'),
       ($1, 'ค่าน้ำ-ไฟ-เน็ต', 'expense'),
       ($1, 'ประกัน', 'expense'),
       ($1, 'สุขภาพ/ความงาม', 'expense'),
       ($1, 'ลูกหนี้', 'expense'),
       ($1, 'ธุรกิจ/การตลาดออนไลน์', 'expense'),
       ($1, 'การศึกษา/พัฒนาตนเอง', 'expense'),
       ($1, 'ส่วนตัว/ท่องเที่ยว/ของขวัญ', 'expense'),
       ($1, 'เงินออม/ลงทุน', 'expense')`,
    [userId]
  );
}
